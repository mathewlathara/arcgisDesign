...
function selectFeature1() {
    ...
    dataColumns = ['STATION', 'CensusYear', 'Year', 'Month', 'DATE', 'Nitrogen_Kjeldahl',
...
function filterRows(row) {
    ...
    while (i < row.length) {
        if (row[i]["Year"] > yearFrom.value && row[i]["Year"] < yearTo.value && row[i]["STATION"] == station_id.value) {
            phosphorus[j] = row[i][f1.value];
            nitrogen[j] = row[i][f2.value];
            years[j] = row[i]["Year"];
    ...
    console.log(phosphorus[2])
    return [years, phosphorus, nitrogen];
}
...
function processData(allRows) {
    ...
    phosphorus = filteredData[1];
    nitrogen = filteredData[2];
    ...
    if (f1.value == "Nitrogen_Kjeldahl") {
        makePlotlyNK(x, y1);
    }
    if (f1.value == "TotalNitrogen") {
        makePlotlyN(x, y1);
    }
    makePlotly2(y1, y2);
}
